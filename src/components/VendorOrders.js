...
const Orders = () => {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const dateFilter = order.orderDate
      ? order.orderDate.toLowerCase().includes(filter.toLowerCase())
      : false;
    const priceFilter = order.orderTotal
      ? order.orderTotal.toString().toLowerCase().includes(filter.toLowerCase())
      : false;
  ...
  const handleCancel = (id) => {
    ...
    if (res) {
      axios
        .put(`https://localhost:7022/api/order/${id}/cancel`)
        .then((response) => {
